...
import { Calendar, Clock, MapPin, Sparkles, ChevronDown, ExternalLink } from 'lucide-react';
    ...
    <section id="hero" className="relative min-h-screen flex items-center justify-center pt-24 pb-16 overflow-hidden bg-[#1a040a]">
      ...
      <div className="absolute inset-0 z-0">
        <img
          ...
          alt="Carole and John Evening Reception"
      ...
      <div className="relative z-10 max-w-4xl mx-auto px-4 sm:px-6 text-center text-rose-50">
        ...
        <h1 className="font-serif text-5xl sm:text-7xl md:text-8xl text-transparent bg-clip-text bg-gradient-to-b from-amber-100 via-rose-200 to-amber-300 font-light tracking-tight mb-4 drop-shadow-2xl">
          Carole <span className="text-amber-300/90 font-serif italic font-normal">&amp;</span> John
        </h1>
        ...
        <div className="flex flex-wrap items-center justify-center gap-4 sm:gap-6 mb-10 text-sm sm:text-base">
          ...
          <button
            ...
            className="flex items-center space-x-2 bg-[#25050f]/80 hover:bg-[#3d0818] border border-rose-800/40 hover:border-amber-400/60 px-4 py-2 rounded-2xl backdrop-blur-md text-rose-100 hover:text-amber-200 transition-all cursor-pointer group shadow-lg"
            title="View Venue Location & Navigation Map"
          >
            <MapPin className="w-4 h-4 text-amber-300 group-hover:scale-110 transition-transform" />
            <span className="font-medium underline decoration-amber-400/40 underline-offset-4 group-hover:decoration-amber-300">{EVENT_DETAILS.venueName}</span>
            <ExternalLink className="w-3.5 h-3.5 text-amber-300/70 group-hover:text-amber-300 shrink-0 ml-0.5" />
          </button>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
          ...
          <button
            onClick={onVenueClick}
            className="w-full sm:w-auto bg-[#2d0714]/90 hover:bg-[#3a091a] text-amber-200 border border-amber-500/50 hover:border-amber-300 px-8 py-4 rounded-full text-sm uppercase tracking-widest backdrop-blur-md transition-all flex items-center justify-center space-x-2"
          >
