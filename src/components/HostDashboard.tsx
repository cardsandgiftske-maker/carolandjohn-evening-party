...
import { X, Download, Search, UserCheck, Music, Trash2, Lock, KeyRound, Eye, EyeOff, ShieldCheck, AlertCircle, LogOut } from 'lucide-react';
...
const VALID_PASSCODES = ['carole2026', 'caroleandjohn', '1234', 'host123', 'carole', 'john2026'];
...
export const HostDashboard: React.FC<HostDashboardProps> = ({
  ...
  const [isAuthenticated, setIsAuthenticated] = useState<boolean>(() => {
    return sessionStorage.getItem('carole_john_host_auth') === 'true';
  });
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (VALID_PASSCODES.includes(cleanPass)) {
      sessionStorage.setItem('carole_john_host_auth', 'true');
  ...
  const handleSignOut = () => {
    sessionStorage.removeItem('carole_john_host_auth');
    setIsAuthenticated(false);
    setPasswordInput('');
    setErrorMessage('');
  };

  // If not authenticated, render Host Lock Portal
  if (!isAuthenticated) {
    return (
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-[#120206]/95 backdrop-blur-lg animate-fadeIn">
        <div className="bg-[#240610] border-2 border-rose-800/60 rounded-3xl w-full max-w-md p-6 sm:p-8 shadow-2xl relative overflow-hidden">
          {/* Ambient Glow */}
          <div className="absolute -top-24 -left-24 w-48 h-48 bg-amber-500/10 rounded-full blur-3xl pointer-events-none" />
          
          <button
            onClick={onClose}
            className="absolute top-4 right-4 p-2 text-rose-300/70 hover:text-rose-100 bg-[#180308] rounded-xl border border-rose-800/40 transition-colors"
          >
            <X className="w-5 h-5" />
          </button>

          <div className="text-center mb-6">
            <div className="w-16 h-16 mx-auto mb-4 bg-gradient-to-tr from-amber-400 to-rose-500 p-0.5 rounded-2xl shadow-xl shadow-amber-500/10">
              <div className="w-full h-full bg-[#180308] rounded-[14px] flex items-center justify-center">
                <Lock className="w-8 h-8 text-amber-300 animate-pulse" />
              </div>
            </div>

            <div className="inline-flex items-center space-x-1.5 bg-[#180308] border border-amber-500/30 px-3 py-1 rounded-full mb-2">
            ...
            <h2 className="font-serif text-2xl text-rose-50 font-medium mt-1">
              Host Security Portal
            </h2>
            <p className="text-xs text-rose-200/70 font-sans mt-1">
              Enter the host passcode to access Carole &amp; John's guest list and RSVP management portal.
            </p>
          </div>

          <form onSubmit={handlePasswordSubmit} className="space-y-4">
            <div>
              <label className="text-xs font-mono text-amber-300/90 uppercase tracking-wider block mb-2">
                Host Passcode:
              </label>
              <div className="relative">
                <KeyRound className="w-4 h-4 text-rose-300/60 absolute left-3.5 top-3.5" />
                <input
                  ...
                  autoFocus
                  className="w-full bg-[#180308] border border-rose-800/60 focus:border-amber-400 rounded-xl pl-10 pr-10 py-3 text-sm text-rose-50 placeholder-rose-300/30 outline-none transition-all font-mono"
                />
                <button
                  type="button"
                  onClick={() => setShowPassword(!showPassword)}
                  className="absolute right-3 top-3 text-rose-300/60 hover:text-amber-200 transition-colors"
                >
            ...
            {errorMessage && (
              <div className="bg-rose-950/80 border border-rose-500/50 rounded-xl p-3 flex items-center space-x-2 text-rose-200 text-xs animate-shake">
            ...
            >
              <ShieldCheck className="w-4 h-4 text-rose-950" />
      ...
      <div className="bg-[#240610] border border-rose-800/50 rounded-3xl w-full max-w-5xl max-h-[90vh] flex flex-col shadow-2xl overflow-hidden">
        ...
        <div className="p-6 bg-[#180308] border-b border-rose-900/40 flex items-center justify-between">
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="font-serif text-2xl text-rose-50 font-medium flex items-center space-x-2">
                <span>Carole &amp; John Host Portal</span>
                <span className="text-[10px] font-mono bg-emerald-950 text-emerald-300 border border-emerald-500/30 px-2 py-0.5 rounded-full uppercase tracking-wider font-normal">
                  Secured
                </span>
              </h2>
          ...
          <div className="flex items-center space-x-3">
            ...
            <button
              ...
              className="p-2 text-rose-300/70 hover:text-amber-200 bg-[#180308] hover:bg-[#2c0814] rounded-xl border border-rose-800/40 transition-colors flex items-center space-x-1 text-xs"
