...
export default function App() {
  ...
  const fetchRsvps = async () => {
    try {
      const firebaseData = await fetchRsvpsFromFirebase();
      if (firebaseData && firebaseData.length > 0) {
        setRsvps(firebaseData);
        return;
      }
  ...
  const handleRSVPSubmitted = (newRsvp: RSVPData) => {
    setRsvps((prev) => {
      const existingIdx = prev.findIndex(r => r.id === newRsvp.id || (r.email && newRsvp.email && r.email.toLowerCase() === newRsvp.email.toLowerCase()));
      if (existingIdx >= 0) {
        const copy = [...prev];
      ...
      {showEnvelope ? (
        ...
        <>
          ...
          <HeroSection
            ...
            onEntertainmentClick={() => handleNavigate('entertainment')}
            onVenueClick={() => handleNavigate('venue')}
          />

          {/* Venue & Interactive Map Navigation Section */}
          <VenueMapSection />

          {/* Party Entertainment Headliners Section */}
