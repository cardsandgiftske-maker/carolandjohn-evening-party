...
app.post('/api/rsvps', (req, res) => {
  ...
  if (!newRsvp || !newRsvp.primaryGuestName || !newRsvp.email) {
    return res.status(400).json({ success: false, error: 'Name and Email are required.' });
  }

  // Check if updating existing by email or id
  const existingIndex = rsvpsStore.findIndex(
    r => r.id === newRsvp.id || r.email.toLowerCase() === newRsvp.email.toLowerCase()
  );
